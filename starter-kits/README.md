# Starter Kits

Repository Path: `-agentforge-governance/starter-kits/README.md`
GitHub URL: https://github.com/agentforgeframework-cpu/-agentforge-governance/blob/main/starter-kits/README.md
Raw URL: https://raw.githubusercontent.com/agentforgeframework-cpu/-agentforge-governance/refs/heads/main/starter-kits/README.md

Git Domain:
github.com/agentforgeframework-cpu/-agentforge-governance

---

# Purpose

This folder contains official AgentForge starter kits.

Starter kits are canonical starting points that demonstrate how AgentForge governance and kit structure are applied in practice.

They are not separate governance authorities.

They implement governance patterns defined in `/governance/`.

---

# Current Starter Kits

## `/tools/`

Starter kit for building lightweight AgentForge tool kits.

---

# Guidance

Starter kits should remain:

- lightweight
- understandable
- operational
- easy to copy
- easy to adapt
- aligned with current governance

---

# Development & Test Environment

- Platform: ChatGPT (Web)
- Model: GPT-5.5
- Date: 2026-05-08

Notes:
- Initial starter kits folder README.
